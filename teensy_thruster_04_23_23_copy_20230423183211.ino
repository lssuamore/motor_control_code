...
void thruster_sub_right(const std_msgs::Int32& thruster_msg_right){ //function that subscribes to get the right thruster value

  thruster_value_right = thruster_msg_right.data;
  
}

void angle_sub_right(const std_msgs::Int32& angle_msg_right){ //function that subscribes to get the right angle value

  angle_value_right = angle_msg_right.data;
  
}

void thruster_sub_left(const std_msgs::Int32& thruster_msg_left){ //function that subscribes to get the left thruster value

  thruster_value_left = thruster_msg_left.data;
  
}

void angle_sub_left(const std_msgs::Int32& angle_msg_left){ //function that subscribes to get the left angle value

  angle_value_left = angle_msg_left.data;
  
}

ros::Subscriber<std_msgs::Int32> sub("/thruster_int_right", &thruster_sub_right);
ros::Subscriber<std_msgs::Int32> sub1("/angle_int_right", &angle_sub_right);
ros::Subscriber<std_msgs::Int32> sub2("/thruster_int_left", &thruster_sub_left);
ros::Subscriber<std_msgs::Int32> sub3("/angle_int_left", &angle_sub_left);
...
/*
Gerneral notes
  - Code made by: Anthony Paradine, Thomas Muir, Taylor Lamorie
  - April 2022
  - Team Amore, LSSU RobotX Club
  - When in autonomus mode this code will read 4 serial inputs, 2 as thrust values and 2 as angle values. 
    It will then run these values and output them to the thrusters whien in autonomus mode.
  - When in manual mode this code will allow for RC control of the thrusters using the 2 joysticks 
    on the RC controller.
  - 5v supply to power the ardunio and raido reciver can be provided through the propulsion
    battries via the pwm to analog converter avoid this while testing because unplugging the
    usb from the programming laptop will no longer function as an e-stop. Only use the signal 
    and ground wires for the PWM.
  - The wiring should be in its final configuration. Only one level converter will be required
    and 4 realays will be sufficent.
*/
...
void auto_control(){
  ...
  angle_msg.data = thruster_value_right;
  pub.publish(&angle_msg);
  angle_msg1.data = thruster_value_left;
  pub1.publish(&angle_msg1);
  ...
  {
    port_auto_speed = abs(thruster_value_left);
   //Eliminates motor controler deadzone between 0 and abs(140) by normalizing between 140 and 255
   //140 = 2.75v and 255 = 5v after running through the 3.3v to 5v board and PWM analog converter
   if (port_auto_speed < 140 && port_auto_speed > 5)
   {
     port_auto_speed = round(port_auto_speed * 1.15) + 140;
   }
    analogWrite(25,port_auto_speed);
    /*if (auto_thrusters_on == 0)
    {
      ...
      {
         delay(1);
         if (i < thruster_value_right)
         {
            analogWrite(24,i);
         }
        if (i < thruster_value_left)
         {
            analogWrite(25,i);
         }         
      }
    ...
    {
      analogWrite(25,thruster_value_left);
      analogWrite(24,thruster_value_right);
    }
  }*/
  }
}
